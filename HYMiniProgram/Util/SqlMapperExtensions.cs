...
namespace HYMiniProgram
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static int GetCount(this IDbConnection connection, object condition, Type type, bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            return QueryList<int>(connection, condition, type, "count(*)", isOr, transaction, commandTimeout).Single();
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static Task<int> GetCountAsync(this IDbConnection connection, object condition, Type type, bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            return QueryListAsync<int>(connection, condition, type, "count(*)", isOr, transaction, commandTimeout).ContinueWith<int>(t => t.Result.Single());
        }
        ...
        ///// <param name="commandTimeout"></param>
        ///// <returns></returns>
        //public static T QuerySingle<T>(this IDbConnection connection, dynamic condition, string table, IDbTransaction transaction = null, int? commandTimeout = null)
        //{
        //    return QueryList<T>(connection, condition, table, "top 1 *", false, transaction, commandTimeout);
        //}
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static IEnumerable<dynamic> QueryList(this IDbConnection connection, dynamic condition, Type type, string columns = "*", bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            return QueryList<dynamic>(connection, condition, type, columns, isOr, transaction, commandTimeout);
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static Task<IEnumerable<dynamic>> QueryListAsync(this IDbConnection connection, dynamic condition, Type type, string columns = "*", bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            return QueryListAsync<dynamic>(connection, condition, type, columns, isOr, transaction, commandTimeout);
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static IEnumerable<T> QueryList<T>(this IDbConnection connection, object condition, Type table, string columns = "*", bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static Task<IEnumerable<T>> QueryListAsync<T>(this IDbConnection connection, object condition, Type type, string columns = "*", bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            return connection.QueryAsync<T>(BuildQuerySQL(condition, type, columns, isOr), condition, transaction, commandTimeout);
        }
        ...
        public static IEnumerable<T> QueryPaged<T>(this IDbConnection connection, dynamic condition, string table, string orderBy, int pageIndex, int pageSize, string columns = "*", bool isOr = false, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (properties.Count > 0)
            {
                ...
                whereFields = " WHERE " + string.Join(separator, properties.Select((p) =>
                {
                    var propInfo = typeof(T).GetProperty(p);
                    return $"{p} = @{p}";
                }));
            }
            //SqlServer
        ...
        private static string BuildQuerySQL(dynamic condition, Type type, string selectPart = "*", bool isOr = false)
        {
            var conditionObj = condition as object;
            var properties = GetProperties(conditionObj);
            if (properties.Count == 0)
            {
                return string.Format("SELECT {1} FROM {0}", type.GetTableName(), selectPart);
            }

            var separator = isOr ? " OR " : " AND ";

            var wherePart = string.Join(separator, properties.Select((p) =>
            {
                var propInfo = type.GetProperty(p);
                return $"{p} = @{p}";
            }));

            return string.Format("SELECT {2} FROM {0} WHERE {1}", type.GetTableName(), wherePart, selectPart);
        }
        private static List<string> GetProperties(object obj)
        {
            ...
            {
                return (obj as DynamicParameters).ParameterNames.Select(x => x).ToList();
            }
            if (obj is Dictionary<string, object>)
            {
                return (obj as Dictionary<string, object>).Select(x => x.Key).ToList();
            }
            return GetPropertyInfos(obj).Select((x) =>
            {
                return x.Name;
            }).ToList();
        }
